...
const DEFAULT_ITEMS: DepthCarouselItem[] = [
  { image: "https://picsum.photos/seed/depth1/800/1000", title: "AI Generation Studio", description: "Automated design engine for modern web apps." },
  { image: "https://picsum.photos/seed/depth2/800/1000", title: "Cyber Canvas Renderer", description: "Real-time WebGL particle starfield visualizer." },
  { image: "https://picsum.photos/seed/depth3/800/1000", title: "3D Depth Physics UI", description: "ActiveTheory style 3D perspective component library." },
  { image: "https://picsum.photos/seed/depth4/800/1000", title: "Quantum Code Engine", description: "High-performance React & TypeScript application stack." },
];

const clamp = (v: number, min: number, max: number) => Math.min(Math.max(v, min), max);

const normalizeItem = (it: string | DepthCarouselItem): DepthCarouselItem =>
  typeof it === "string" ? { image: it, alt: "", title: "Featured Item" } : it;
  ...
  useEffect(() => {
    ...
    const start = () => {
      ...
      autoTimerRef.current = window.setInterval(
        ...
        Math.max((cfgRef.current as { autoplayDelay: number }).autoplayDelay || 3200, 600)
      );
    };
        ...
        {data.map((item, i) => (
          ...
          >
            {item.image ? (
              ...
              <div className="w-full h-full bg-gradient-to-br from-[#0e0e14] to-[#161620] flex items-center justify-center p-6 text-center border border-white/10">
                <span className="text-4xl font-extrabold text-[var(--p-primary,#00f0ff)]">{(item.title || "Item").charAt(0)}</span>
              </div>
            )}
            ...
            {(item.title || item.description) && (
              <div className="depth-carousel__content">
                {item.title && (
                  <h3 className="text-lg font-black text-white group-hover:text-[var(--p-primary,#00f0ff)] transition-colors leading-tight mb-1">
                    {item.title}
                  </h3>
                )}
                {item.description && (
                ...
                {item.tags && item.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-3">
                    {item.tags.slice(0, 3).map((tag) => (
                      <span key={tag} className="px-2 py-0.5 text-[10px] font-bold rounded-full bg-white/15 text-white border border-white/10">
                        {tag}
                      </span>
                    ))}
                  </div>
