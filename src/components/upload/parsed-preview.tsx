    ...
    <div className="w-full">
      <div className="flex items-start sm:items-center justify-between mb-4 flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-base font-semibold text-white">Resume parsed successfully</h3>
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 text-xs font-medium text-text-muted hover:text-white transition-colors cursor-pointer"
        >
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 mb-4">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h4 className="text-2xl font-bold text-white">{personal.name ?? "Unknown"}</h4>
            {(personal.role || personal.headline) && (
              <p className="text-sm text-primary mt-1">{personal.role ?? personal.headline}</p>
            )}
            {personal.location && (
          ...
          <div className="flex gap-2 flex-wrap">
            {personal.email && (
              <span className="text-[10px] text-text-muted bg-white/5 rounded-full px-2.5 py-1">{personal.email}</span>
            )}
            {personal.linkedin && (
        ...
        {personal.summary && (
          <p className="text-sm text-text-muted leading-relaxed mt-3 line-clamp-3">{personal.summary}</p>
        )}
      </div>
      ...
      {strategy && (
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
          <p className="text-[10px] text-text-muted uppercase tracking-wider mb-2">Portfolio Strategy</p>
          <div className="flex flex-wrap gap-2">
            <span className="text-xs font-medium text-white bg-primary/15 border border-primary/30 rounded-full px-3 py-1">
              {strategy.portfolioType.replace("-", " ")}
            </span>
            <span className="text-xs font-medium text-white bg-white/5 border border-white/10 rounded-full px-3 py-1">
